...
@section('title', 'Crear Reunión del Consejo')
...
<div class="mb-6">
    ...
    <h1 class="text-3xl font-bold text-gray-900">Crear Reunión del Consejo</h1>
</div>
...
<form action="{{ route('admin.consejo-reuniones.store') }}" method="POST" class="space-y-6" id="formReunion">
    @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="tipo_reunion" id="tipo_reunion" required
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('tipo_reunion') border-red-500 @enderror">
                    <option value="">Seleccione un tipo</option>
                    <option value="ordinaria" {{ old('tipo_reunion') == 'ordinaria' ? 'selected' : '' }}>Ordinaria</option>
                    <option value="extraordinaria" {{ old('tipo_reunion') == 'extraordinaria' ? 'selected' : '' }}>Extraordinaria</option>
                </select>
                @error('tipo_reunion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div id="campo_lugar" style="display: none;">
                <label for="lugar" class="block text-sm font-medium text-gray-700 mb-1">
                    Lugar
                </label>
                <input type="text" name="lugar" id="lugar" value="{{ old('lugar') }}"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('lugar') border-red-500 @enderror"
            ...
            <div id="campo_enlace" style="display: none;">
                <label for="enlace_virtual" class="block text-sm font-medium text-gray-700 mb-1">
                    Enlace Virtual
                </label>
                <input type="url" name="enlace_virtual" id="enlace_virtual" value="{{ old('enlace_virtual') }}"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('enlace_virtual') border-red-500 @enderror"
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div id="agenda-container" class="space-y-4">
            <!-- Los puntos de agenda se agregarán aquí dinámicamente -->
        </div>
    </div>
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="integrantes[]" id="integrante_{{ $integrante->id }}" value="{{ $integrante->id }}"
                    {{ in_array($integrante->id, old('integrantes', [])) ? 'checked' : '' }}
...
function agregarPuntoAgenda() {
    ...
    puntoDiv.innerHTML = `
        <div class="flex items-start justify-between mb-3">
            <span class="text-sm font-medium text-gray-700">Punto ${puntosAgenda.length}</span>
...
// Inicializar campos según modalidad seleccionada
