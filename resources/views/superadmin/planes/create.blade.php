...
@section('title', 'Crear Plan - SuperAdmin')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Crear Nuevo Plan</h1>
    <p class="mt-2 text-sm text-gray-600">Agrega un nuevo plan al sistema</p>
</div>

<form action="{{ route('superadmin.planes.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <input 
                ...
                value="{{ old('max_almacenamiento_mb') }}" 
                min="0"
                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Dejar vacío para ilimitado"
            >
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center">
                <input 
                    ...
                    value="1"
                    {{ old('soporte_prioritario') ? 'checked' : '' }}
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                >
