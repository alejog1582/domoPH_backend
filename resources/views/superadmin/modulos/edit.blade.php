...
@section('title', 'Editar Módulo - SuperAdmin')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Editar Módulo</h1>
    <p class="mt-2 text-sm text-gray-600">Modifica la información del módulo</p>
</div>

<form action="{{ route('superadmin.modulos.update', $modulo) }}" method="POST" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $modulo->nombre) }}" required
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $modulo->slug) }}" required
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descripcion" id="descripcion" rows="3"
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $modulo->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="icono" id="icono" value="{{ old('icono', $modulo->icono) }}"
            ...
            <div>
                ...
                <input type="text" name="ruta" id="ruta" value="{{ old('ruta', $modulo->ruta) }}"
            ...
            <div>
                ...
                <input type="number" name="orden" id="orden" value="{{ old('orden', $modulo->orden) }}" min="0"
            ...
            <div>
                ...
                <select name="activo" id="activo"
                    ...
                    <option value="1" {{ old('activo', $modulo->activo) == '1' || old('activo', $modulo->activo) == 1 ? 'selected' : '' }}>Activo</option>
                    <option value="0" {{ old('activo', $modulo->activo) == '0' || old('activo', $modulo->activo) == 0 ? 'selected' : '' }}>Inactivo</option>
                </select>
                @error('activo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="requiere_configuracion" id="requiere_configuracion"
                    ...
                    <option value="0" {{ old('requiere_configuracion', $modulo->requiere_configuracion) == '0' || old('requiere_configuracion', $modulo->requiere_configuracion) == 0 ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('requiere_configuracion', $modulo->requiere_configuracion) == '1' || old('requiere_configuracion', $modulo->requiere_configuracion) == 1 ? 'selected' : '' }}>Sí</option>
                </select>
            ...
            <div>
                <label for="es_admin" class="block text-sm font-medium text-gray-700 mb-2">
                    Usado por Admin <span class="text-red-500">*</span>
                </label>
                <select name="es_admin" id="es_admin" required
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('es_admin') border-red-500 @enderror">
                    <option value="">Seleccione una opción</option>
                    <option value="1" {{ old('es_admin', $modulo->es_admin) == '1' || old('es_admin', $modulo->es_admin) == 1 ? 'selected' : '' }}>Sí</option>
                    <option value="0" {{ old('es_admin', $modulo->es_admin) == '0' || old('es_admin', $modulo->es_admin) == 0 ? 'selected' : '' }}>No</option>
                </select>
                <p class="mt-1 text-xs text-gray-500">Indica si el módulo es exclusivo para administradores</p>
                @error('es_admin')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="configuracion_default" id="configuracion_default" rows="6"
                    ...
                    placeholder='{"clave": "valor"}'>{{ old('configuracion_default') ? (is_string(old('configuracion_default')) ? old('configuracion_default') : json_encode(old('configuracion_default'), JSON_PRETTY_PRINT)) : ($modulo->configuracion_default ? json_encode($modulo->configuracion_default, JSON_PRETTY_PRINT) : '') }}</textarea>
