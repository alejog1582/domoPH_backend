...
@section('title', 'Depósitos - Administrador')
...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Gestión de Depósitos</h1>
            <p class="mt-2 text-sm text-gray-600">Administración de depósitos de la propiedad</p>
        </div>
        <a href="{{ route('admin.depositos.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition">
...
@if($propiedad)
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">
            Sección para cargar la información de los depósitos
        </h2>
        ...
        <div class="mb-4">
            ...
            <ul class="list-disc list-inside text-gray-600 space-y-1 mb-4">
                <li><strong>Paso 1:</strong> Descargue la plantilla de ejemplo en excel <a href="{{ route('admin.depositos.template') }}" class="text-red-600 hover:text-red-800 underline font-semibold">aquí</a>.</li>
                <li><strong>Paso 2:</strong> Seleccione el archivo con la información y haga clic en el botón importar:</li>
            </ul>
        </div>

        <form action="{{ route('admin.depositos.import') }}" method="POST" enctype="multipart/form-data" class="mb-4">
            @csrf
        ...
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
            <p class="text-sm">
                <strong>Nota importante:</strong> Cada cargue que realiza reemplaza la información previamente cargada, asegúrese de no borrar la información. <strong>Solo se editarán los datos de los depósitos que estén en el archivo cargado.</strong>
            </p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <form method="GET" action="{{ route('admin.depositos.index') }}">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                    @if($deposito->estado == 'disponible') bg-green-100 text-green-800
                                    @elseif($deposito->estado == 'asignado') bg-blue-100 text-blue-800
                                    @elseif($deposito->estado == 'en_mantenimiento') bg-yellow-100 text-yellow-800
                                    @else bg-red-100 text-red-800
                                    @endif">
                                    {{ ucfirst(str_replace('_', ' ', $deposito->estado)) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $deposito->area_m2 ? number_format($deposito->area_m2, 2) : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @if($deposito->unidad)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @if($deposito->residenteResponsable)
                                    {{ $deposito->residenteResponsable->user->nombre ?? 'N/A' }}
                                @else
                                    <span class="text-gray-400">-</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.depositos.destroy', $deposito->id) }}" 
                                        method="POST" 
                                        onsubmit="return confirm('¿Está seguro de eliminar este depósito?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-500">No hay depósitos registrados</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
