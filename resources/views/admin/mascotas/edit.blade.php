...
@section('title', 'Editar Mascota - Administrador')
...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-600">Modifica la información de la mascota</p>
        </div>
...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.mascotas.update', $mascota->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                >
                    ...
                    @foreach($unidades as $unidad)
                        ...
                        @endphp
                        <option value="{{ $unidad->id }}" {{ old('unidad_id', $mascota->unidad_id) == $unidad->id ? 'selected' : '' }} data-residentes="{{ json_encode($unidad->residentes->map(function($r) { return ['id' => $r->id, 'nombre' => $r->user->nombre ?? 'Sin nombre']; })) }}">
                            {{ $unidadLabel }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                >
                    ...
                    <option value="reptil" {{ old('tipo', $mascota->tipo) == 'reptil' ? 'selected' : '' }}>Reptil</option>
                    <option value="roedor" {{ old('tipo', $mascota->tipo) == 'roedor' ? 'selected' : '' }}>Roedor</option>
            ...
            <div>
                ...
                >
                    ...
                    <option value="desconocido" {{ old('sexo', $mascota->sexo) == 'desconocido' ? 'selected' : '' }}>Desconocido</option>
                </select>
                @error('sexo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input 
                    ...
                    id="peso_kg" 
                    name="peso_kg" 
                    value="{{ old('peso_kg', $mascota->peso_kg) }}" 
                    step="0.01"
                    min="0"
            ...
            <div>
                ...
                >
                    ...
                    <option value="pequeño" {{ old('tamanio', $mascota->tamanio) == 'pequeño' ? 'selected' : '' }}>Pequeño</option>
                    <option value="mediano" {{ old('tamanio', $mascota->tamanio) == 'mediano' ? 'selected' : '' }}>Mediano</option>
                    <option value="grande" {{ old('tamanio', $mascota->tamanio) == 'grande' ? 'selected' : '' }}>Grande</option>
                </select>
                @error('tamanio')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input 
                    ...
                    id="numero_chip" 
                    name="numero_chip" 
                    value="{{ old('numero_chip', $mascota->numero_chip) }}" 
            ...
            <div>
                ...
                >
                    ...
                    <option value="saludable" {{ old('estado_salud', $mascota->estado_salud) == 'saludable' ? 'selected' : '' }}>Saludable</option>
                    <option value="en_tratamiento" {{ old('estado_salud', $mascota->estado_salud) == 'en_tratamiento' ? 'selected' : '' }}>En Tratamiento</option>
                    <option value="crónico" {{ old('estado_salud', $mascota->estado_salud) == 'crónico' ? 'selected' : '' }}>Crónico</option>
                    <option value="desconocido" {{ old('estado_salud', $mascota->estado_salud) == 'desconocido' ? 'selected' : '' }}>Desconocido</option>
                </select>
                @error('estado_salud')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input 
                    ...
                    value="{{ old('fecha_vigencia_vacunas', $mascota->fecha_vigencia_vacunas ? $mascota->fecha_vigencia_vacunas->format('Y-m-d') : '') }}" 
            ...
            <div>
                <label class="flex items-center">
                    <input 
                        ...
                        name="activo" 
                        value="1"
                        {{ old('activo', $mascota->activo) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                    >
            ...
            <div class="md:col-span-2">
                ...
                >{{ old('observaciones', $mascota->observaciones) }}</textarea>
                @error('observaciones')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                @if($mascota->foto_url)
                    <div class="mb-3">
                        <img src="{{ $mascota->foto_url }}" alt="Foto actual" class="h-32 w-32 object-cover rounded-md border border-gray-300">
                        <p class="mt-2 text-xs text-gray-500">Foto actual</p>
                    </div>
                @endif
                <input 
                    type="file" 
            ...
            <div class="md:col-span-2">
                ...
                @if($mascota->foto_url_vacunas)
                    <div class="mb-3">
                        <img src="{{ $mascota->foto_url_vacunas }}" alt="Foto de vacunación actual" class="h-32 w-32 object-cover rounded-md border border-gray-300">
                        <p class="mt-2 text-xs text-gray-500">Foto de vacunación actual</p>
                    </div>
                @endif
                <input 
                    type="file" 
...
<script>
    function cargarResidentes(unidadId) {
        ...
        if (unidadId && optionSeleccionada.dataset.residentes) {
            try {
                ...
                const residenteIdActual = '{{ old("residente_id", $mascota->residente_id) }}';
                
                residentes.forEach(function(residente) {
                    const option = document.createElement('option');
                    option.value = residente.id;
                    option.textContent = residente.nombre;
                    if (residente.id == residenteIdActual) {
    ...
    // Cargar residentes al cargar la página
